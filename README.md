...
```bash
multi-step-workflow-tf$ sam build
```
...
```bash
multi-step-workflow-tf$ sam logs -n StockCheckerFunction --stack-name multi-step-workflow-tf --tail
```
...
```bash
multi-step-workflow-tf$ cd functions/<function-name>/
...
```bash
sam delete --stack-name multi-step-workflow-tf
```
